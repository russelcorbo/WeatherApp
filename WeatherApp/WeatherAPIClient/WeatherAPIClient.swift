...
final class WeatherAPIClient: NSObject, ObservableObject, CLLocationManagerDelegate {
    ...
    func fetchWeather() async {
        ...
        guard let url = URL(string: "https://api.tomorrow.io/v4/timelines?location=\(location.coordinate.latitude),\(location.coordinate.longitude)&fields=temperature&fields=weatherCode&units=metric&timesteps=1h&startTime=\(dateFormatter.string(from: Date()))&endTime=\(dateFormatter.string(from: Date().addingTimeInterval(60 * 60)))&apikey={Q99Xulz4YwkNtt93sjsy6aFktXrzIMMT}") else {
            return
        }
            ...
            if let weatherResponse = try? JSONDecoder().decode(WeatherModel.self, from: data),
               ...
               let weatherCode = WeatherCode(rawValue: "\(weatherValue.weatherCode)") {
                DispatchQueue.main.async { [weak self] in
                    self?.currentWeather = Weather(temperature: Int(weatherValue.temperature), weatherCode: weatherCode)
                }
            }
        } catch {
            // handle the error
